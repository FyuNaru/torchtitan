...
class Batcher(Configurable):
    ...
    def _pack_one_training_batch(self) -> TrainingBatch:
        ...
        if len(assignments) > len(baseline_assignments):
    ...
    def _assign_training_samples_to_microbatches(
        ...
        """Balance samples by token count across fixed-capacity DP inputs.

        Samples are placed longest-first into the least-loaded eligible DP rank.
        A new microbatch is added only when no existing rank has capacity.
        """
        ...
        for training_sample in ordered_samples:
            ...
            eligible_ranks = [
                ...
                if rank_num_tokens[microbatch][rank] + num_tokens <= num_tokens_per_rank
                and (
            ...
            if not eligible_ranks:
                ...
                eligible_ranks = [(len(microbatches) - 1, 0)]

            microbatch, rank = min(
                eligible_ranks,
                key=lambda cell: (rank_num_tokens[cell[0]][cell[1]], cell),
            )
