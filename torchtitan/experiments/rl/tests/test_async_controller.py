...
def test_batcher_balances_tokens_across_dp_ranks() -> None:
    batcher = Batcher.Config().build(
        num_tokens_per_microbatch_per_dp_rank=8,
...
def test_batcher_does_not_add_padding_when_document_limit_blocks_greedy_order() -> None:
    ...
    expected_padding_frac = pytest.approx(1 / 6)
    assert (
        _metric_value(batch, "train_batch/padding_frac_before_load_balance")
        == expected_padding_frac
    )
    assert _metric_value(batch, "train_batch/padding_frac") == expected_padding_frac
